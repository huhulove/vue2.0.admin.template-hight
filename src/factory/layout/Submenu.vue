<template>
	<el-submenu :index="'' + menu_p.id">
		<template slot="title">
			...
			<span style="padding-left: 12px">{{ menu_p.menuName }}</span>
		</template>
		<template v-for="($menu, $index) in menu_p.children">
			<span :key="$index">
				<el-menu-item :index="`${$menu.id}`" :route="{ path: `/${$menu.frontendRoute ? $menu.frontendRoute.path : '404'}` }" @click.native="clickMenu($menu)">
					<svg-icon :icon-class="$menu.menuIcon" class="el-input__icon" style="height: 16px; width: 16px; vertical-align: middle" />
					<span style="padding-left: 12px;">{{ $menu.menuName }}</span>
				</el-menu-item>
