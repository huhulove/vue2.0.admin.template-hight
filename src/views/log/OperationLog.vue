<template>
	<div>
		...
		<Dialog title="日志详情" :visible.sync="isShowLog" width="70%">
			<LogDetail v-if="isShowLog" :logId_p="logId" logType_p=2></LogDetail>
		</Dialog>
	</div>
</template>
...
import { operateLogService } from '@s/log/logService';
...
export default {
	...
	methods: {
		...
		async getLog() {
			...
			const res = await operateLogService(this.dataJson);
